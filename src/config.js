...
// 사용자 해상도 슬라이더(배율)를 줌별 기본 패치 계획에 적용한다.
// 기본 samples/refineSamples를 배율만큼 늘리거나 줄인다(배율 1.0 = 기본값 유지).
// 타일 원본 해상도(256px)를 넘는 샘플링은 의미가 없어 상한을, 너무 거친 메시를 막아 하한을 둔다.
export function scalePlanResolution(plan, scale) {
  if (!scale || scale === 1) return plan;
  const scaleSamples = (n) => {
    const scaled = Math.round((n - 1) * scale) + 1;
    return Math.max(8, Math.min(TILE_SIZE, scaled));
  };
  const samples = scaleSamples(plan.samples);
  const refineSamples = Math.max(samples, scaleSamples(plan.refineSamples));
  return { ...plan, samples, refineSamples };
}
